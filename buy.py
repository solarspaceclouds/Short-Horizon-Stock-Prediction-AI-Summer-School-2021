...
def predictType():
    ...
    SMA_short = getPrice().rolling_window(5).mean()
    SMA_long = getPrice().rolling_window(15).mean()
    ...
    #EMA_short = np.exp(getPrice().rolling_window(5)).mean()
    #EMA_long = np.exp(getPrice().rolling_window(15)).mean()
    #prediction = np.where(EMA_short>EMA_long,1,-1)
    #if prediction == 1:
    #    return 'put'
    #else:
    #    return 'call'
